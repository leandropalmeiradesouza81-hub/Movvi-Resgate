import{O as we,D as se,a as je,A as ke}from"./api-H6lC6YHp.js";import{g as _e,i as Ue}from"./geo-Dl6lxGpH.js";let ue,P,le,c,a,N,F,me,re,Ee=null,Y=localStorage.getItem("movvi_weather")!=="off",G=localStorage.getItem("movvi_traffic")!=="off",C=localStorage.getItem("movvi_map_traffic")!=="off",K=localStorage.getItem("movvi_news")!=="off";const Ye="https://www.myinstants.com/media/sounds/uber.mp3",Ge="https://actions.google.com/sounds/v1/communications/incoming_phone_call.ogg",$=new Audio(Ye);$.loop=!0;$.volume=1;const U=new Audio(Ge);U.loop=!0;U.volume=1;function ie(){(()=>{$.play().then(()=>{$.pause(),$.currentTime=0,console.log("Audio unlocked")}).catch(t=>console.log("Audio unlock failed",t)),U.play().then(()=>{U.pause(),U.currentTime=0}).catch(()=>{})})()}const xe=()=>{ie(),document.removeEventListener("click",xe),document.removeEventListener("touchstart",xe)};document.addEventListener("click",xe,{once:!0});document.addEventListener("touchstart",xe,{once:!0});function Je(){const e=new Date().getHours(),t=new Date().getDay(),i=t===0||t===6;let l,o,r;return i?e>=10&&e<=14?(l="moderado",o="Trânsito Moderado",r="Saídas para lazer podem causar lentidão"):e>=17&&e<=20?(l="moderado",o="Trânsito Moderado",r="Retorno do fim de semana"):(l="livre",o="Trânsito Livre",r="Vias com bom fluxo"):e>=7&&e<=9?(l="intenso",o="Trânsito Intenso",r="Horário de pico matutino"):e>=11&&e<=13?(l="moderado",o="Trânsito Moderado",r="Horário de almoço"):e>=17&&e<=19?(l="intenso",o="Trânsito Intenso",r="Horário de pico vespertino"):e>=20&&e<=22?(l="moderado",o="Trânsito Moderado",r="Fluxo moderado noturno"):(l="livre",o="Trânsito Livre",r="Vias com bom fluxo"),{level:l,label:o,tip:r,color:{livre:"#22c55e",moderado:"#eab308",intenso:"#ef4444"}[l]}}function R(e){a=e,localStorage.setItem("movvi_driver",JSON.stringify(e))}function Ke(){try{return JSON.parse(localStorage.getItem("movvi_driver"))}catch{return null}}function x(e,t){Se(),Ee=e,ue.innerHTML="";const i=e(t);ue.appendChild(i),i.classList.add("fade-in")}function fe(){return`https://mt{s}.google.com/vt/lyrs=${C?"m,traffic":"m"}&x={x}&y={y}&z={z}`}function Xe(e){document.documentElement.classList.remove("dark"),localStorage.setItem("movvi_theme","light"),P&&V()}function Me(){const e=fe();F&&F.eachLayer(t=>{t instanceof L.TileLayer&&t.setUrl(e)})}Xe();const Ze={0:"Céu limpo",1:"Parcialmente limpo",2:"Parcialmente nublado",3:"Nublado",45:"Nevoeiro",48:"Nevoeiro gelado",51:"Garoa leve",53:"Garoa moderada",55:"Garoa densa",61:"Chuva leve",63:"Chuva moderada",65:"Chuva forte",71:"Neve leve",73:"Neve moderada",75:"Neve intensa",80:"Pancadas leves",81:"Pancadas moderadas",82:"Pancadas fortes",95:"Trovoada",96:"Trovoada com granizo leve",99:"Trovoada com granizo forte"};async function We(e,t){try{return await(await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${e}&longitude=${t}&current_weather=true&hourly=temperature_2m,weathercode&forecast_days=1&timezone=America%2FSao_Paulo`)).json()}catch{return null}}function V(){var s;let e=((s=a==null?void 0:a.name)==null?void 0:s.split(" ")[0])||"Motorista";e=e.replace(/\uFFFD/g,"ã").replace(/Joo/i,"João").replace(/Jo.?o/i,"João").replace(/Cambao/i,"Cambão"),P.innerHTML=`
    ...
    <nav class="flex-1 overflow-y-auto pt-4 px-3">
      ...
      <div class="mt-3 mx-1 pt-3 border-t border-slate-200 dark:border-white/10">
        ...
        <div class="flex items-center justify-between px-2 py-2">
          ...
          <button id="map-traffic-toggle" class="relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ${C?"bg-primary":"bg-slate-400 dark:bg-slate-600"}">
            <span class="${C?"translate-x-4":"translate-x-0"} pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow transition duration-200"></span>
          </button>
        </div>
        <div class="flex items-center justify-between px-2 py-2">
    ...
    </div>`,P.querySelectorAll("[data-nav]").forEach(p=>{p.onclick=()=>{ie();const b=p.dataset.nav;b==="dash"?x(T):b==="earn"?x(He):b==="history"?x(et):b==="profile"?x(tt):b==="support"?x(De):b==="logout"&&(c==null||c.emit("driver:offline",a.id),c==null||c.disconnect(),localStorage.removeItem("movvi_driver"),x(ge))}}),P.querySelector(".sidebar-close").onclick=Se;const t=P.querySelector("#weather-toggle");t&&(t.onclick=()=>{Y=!Y,localStorage.setItem("movvi_weather",Y?"on":"off"),V()});const i=P.querySelector("#traffic-toggle");i&&(i.onclick=()=>{G=!G,localStorage.setItem("movvi_traffic",G?"on":"off"),V()});const l=P.querySelector("#map-traffic-toggle");l&&(l.onclick=()=>{C=!C,localStorage.setItem("movvi_map_traffic",C?"on":"off"),V(),Me()});const o=P.querySelector("#news-toggle");o&&(o.onclick=()=>{K=!K,localStorage.setItem("movvi_news",K?"on":"off"),V()});const r=P.querySelector("#btn-sound-test");r&&(r.onclick=()=>{$.currentTime=0,$.play().then(()=>{setTimeout(()=>{$.pause(),$.currentTime=0},2e3)}).catch(()=>alert("Áudio bloqueado! Clique em qualquer lugar e tente novamente."))})}function Ae(){const e=document.createElement("div");e.id="debt-modal-overlay",e.className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm animate-fade-in",e.innerHTML=`
  ...
  `,document.body.appendChild(e),e.querySelector("#modal-request-release").onclick=()=>{c.emit("chat:driver-to-admin",{driverId:a.id,message:`⚠️ *SOLICITAÇÃO DE LIBERAÇÃO*

Por favor, peço a liberação do meu acesso na ferramenta após pagamento do débito pendente.`});const t=e.querySelector("#modal-request-release");t.innerHTML='<span class="material-symbols-outlined text-[18px]">check_circle</span> Solicitação Enviada',t.classList.add("opacity-70","pointer-events-none")},e.querySelector("#modal-check").onclick=async()=>{const t=e.querySelector("#modal-check");t.innerHTML='<div class="size-5 border-2 border-black border-t-transparent rounded-full animate-spin mx-auto"></div>';try{const i=await se.get(a.id);a.blocked=i.blocked,a.walletBalance=i.walletBalance,R(a),!a.blocked&&a.walletBalance>-50?(e.remove(),x(T)):(t.innerHTML="Ainda Bloqueado",setTimeout(()=>t.innerHTML="Verificar Novamente",2e3))}catch{t.innerHTML="Erro ao Sincronizar",setTimeout(()=>t.innerHTML="Verificar Novamente",2e3)}},e.querySelector("#modal-pay").onclick=()=>{e.remove(),x(He)},e.querySelector("#modal-close").onclick=()=>{e.remove()}}function de(){P.classList.add("open"),le.classList.add("open")}function Se(){P.classList.remove("open"),le.classList.remove("open")}function ce(){c||(c=io(),c.on("connect",()=>{a&&(c.emit("register:driver",a.id),a.online&&(c.emit("driver:online",a.id),Pe()))}),c.on("driver:online:error",e=>{e.error==="DEBT_BLOCK"&&(a.online=!1,a.blocked=!0,R(a),Ae(),x(T))}),c.on("driver:data-updated",e=>{console.log("[Socket] Driver data updated from server:",e),a.blocked=e.blocked,a.walletBalance=e.walletBalance,R(a);const t=document.getElementById("debt-modal-overlay");!a.blocked&&(a.walletBalance||0)>-50&&t&&(t.remove(),Ee===T&&x(T))}),setInterval(()=>{c&&c.connected&&a&&a.online&&(c.emit("driver:online",a.id),_e().then(e=>{c.emit("driver:location",{driverId:a.id,latitude:e.latitude,longitude:e.longitude})}).catch(()=>{}))},1e4),c.on("order:incoming",e=>{N||x(Qe,e)}),c.on("order:timeout",()=>{clearInterval(re),$.pause(),$.currentTime=0,N||x(T)}),c.on("order:status",({orderId:e,status:t})=>{t==="cancelled"?($.pause(),$.currentTime=0,clearInterval(re),N=null,x(T)):N&&(N.orderId===e||N.id===e)&&(N.status=t,t==="completed"&&x(Re,N))}),c.on("order-chat:new-message",e=>{const t=document.getElementById("btn-chat");t&&!document.getElementById("chat-msgs")&&(document.getElementById("chat-dot")||(t.style.position="relative",t.innerHTML+='<div id="chat-dot" class="absolute -top-1 -right-1 size-3.5 bg-red-500 rounded-full border-2 border-white dark:border-background-dark animate-pulse"></div>'))}),Ue(({latitude:e,longitude:t})=>{a.online&&c.emit("driver:location",{driverId:a.id,latitude:e,longitude:t}),me&&me.setLatLng([e,t]),F&&F.panTo([e,t])}))}async function Pe(){try{const e=await we.list({driverId:a.id,status:"accepted,pickup,in_progress"});if(e&&e.length>0){const t=e[0];N={orderId:t.id,...t},x(Le,N)}else x(T)}catch(e){console.error("Erro checkActiveOrder:",e)}}function ge(){const t=new URLSearchParams(window.location.search).get("invite")==="pioneer",i=document.createElement("div");return i.className="view active",i.style.background="#FFD900",i.innerHTML=`
...
</style>`,i.querySelector("#lf").onsubmit=async l=>{l.preventDefault(),ie();try{const{user:o}=await ke.loginDriver(i.querySelector("#le").value,i.querySelector("#lp").value);R(o),ce(),V(),!o.approved&&o.onboardingStep!=="approved"?x(ne):x(T)}catch(o){const r=i.querySelector("#le-err");r.textContent=o.message,r.classList.remove("hidden")}},i.querySelector("#go-reg").onclick=()=>x(Ne),i.querySelector("#forgot-pw").onclick=async()=>{const l=window.prompt("Para redefinir sua senha, informe seu email:");if(l)try{await ke.forgotPasswordDriver(l),window.alert(`Um link de recuperação de senha foi enviado para ${l}. Verifique sua caixa de entrada.`)}catch(o){window.alert(o.message||"Erro ao tentar redefinir senha.")}},i}function Ne(){const e=document.createElement("div");return e.className="view active bg-white dark:bg-[#111]",e.innerHTML=`
<div class="flex flex-col relative overflow-hidden" style="min-height:100dvh;font-family:Outfit,Inter,sans-serif">
...
</div>`,e.querySelector("#bk").onclick=()=>x(ge),e.querySelector("#rf").onsubmit=async t=>{t.preventDefault();try{const{user:i}=await ke.registerDriver({name:e.querySelector("#rn").value,email:e.querySelector("#re").value,phone:e.querySelector("#rph").value,pixKey:e.querySelector("#rpix").value,vehicle:e.querySelector("#rv").value,plate:e.querySelector("#rpl").value,password:e.querySelector("#rp").value});R(i),ce(),V(),!i.approved&&i.onboardingStep!=="approved"?x(ne):x(T)}catch(i){const l=e.querySelector("#re-err");l.textContent=i.message,l.classList.remove("hidden")}},e}function ne(){const e=document.createElement("div");e.className="view active bg-background-light dark:bg-background-dark";const t=a.onboardingStep||"documents",i=()=>`
  ...
  `;return e.innerHTML=`
    ...
    <main class="w-full max-w-md mx-auto p-6 pb-24 flex flex-col items-stretch" id="ob-container">
      ${t==="documents"?i():t==="kit"?l():o()}
    </main>
  `,setTimeout(()=>{t==="documents"?e.querySelector("#docs-form").onsubmit=async r=>{r.preventDefault();try{await se.update(a.id,{onboardingStep:"kit",cnhStatus:"submitted",crlvStatus:"submitted"}),a.onboardingStep="kit",R(a),x(ne)}catch(s){alert(s.message)}}:t==="kit"?(e.querySelector("#btn-pay").onclick=()=>{const r=e.querySelector("#pix-modal");r.classList.remove("hidden"),setTimeout(()=>r.querySelector("#pix-modal-content").classList.replace("scale-95","scale-100"),10)},e.querySelector("#btn-cancel-pix").onclick=()=>e.querySelector("#pix-modal").classList.add("hidden"),e.querySelector("#btn-copy-pix").onclick=()=>{const r=e.querySelector("#btn-copy-pix");r.textContent="Chave PIX Copiada!",r.classList.add("opacity-50"),setTimeout(()=>{e.querySelector("#pix-modal-content").innerHTML=`
           ...
           `,setTimeout(async()=>{try{await se.update(a.id,{onboardingStep:"pending_approval",kitAcquired:!0}),a.onboardingStep="pending_approval",a.kitAcquired=!0,R(a),x(ne)}catch(s){alert(s.message)}},3500)},3e3)}):(e.querySelector("#btn-refresh").onclick=async()=>{try{const r=await se.get(a.id);R(r),r.approved?x(T):(alert("Sua conta ainda está em análise. Fique de olho, em breve você estará online!"),x(ne))}catch{}},e.querySelector("#btn-request-release").onclick=()=>{c.emit("chat:driver-to-admin",{driverId:a.id,message:`⚠️ *NOVO CANDIDATO - SOLICITAÇÃO DE LIBERAÇÃO*

Já paguei pelo Kit Resgate e estou aguardando a liberação na plataforma.`});const r=e.querySelector("#btn-request-release");r.innerHTML='<span class="material-symbols-outlined text-[18px]">check_circle</span> Solicitação Enviada',r.classList.add("opacity-70","pointer-events-none")},e.querySelector("#btn-logout").onclick=()=>{localStorage.removeItem("movvi_driver"),a=null,x(ge)})},0),e}function T(){const e=document.createElement("div");e.className="view active";const t=a.online||!1;e.innerHTML=`
<div class="relative flex h-full w-full flex-col overflow-hidden" style="height:100dvh">
  ...
  <section id="bottom-sheet" class="relative z-30 bg-white dark:bg-background-dark border-t border-slate-200 dark:border-primary/20 rounded-t-[2rem] shadow-[0_-10px_30px_rgba(0,0,0,0.15)] dark:shadow-[0_-10px_30px_rgba(0,0,0,0.5)] transition-transform duration-300 ease-out" style="touch-action:none;">
    ...
    <div id="sheet-content" class="px-5 pb-5 pt-0 overflow-hidden transition-all duration-300">
      ...
      <div id="sheet-details">
        ...
        <div id="news-section" class="mt-3">
          ...
          <div id="news-ticker" class="relative overflow-hidden" style="min-height:48px">
...
</div>`,e.querySelector("#btn-menu").onclick=de,e.querySelector("#bottom-sheet");const i=e.querySelector("#sheet-details"),l=e.querySelector("#sheet-arrow"),o=e.querySelector("#sheet-handle");let r=!1;function s(){r=!0,i&&(i.style.maxHeight="0px",i.style.opacity="0",i.style.overflow="hidden"),l&&(l.style.transform="rotate(180deg)")}function p(){r=!1,i&&(i.style.maxHeight="600px",i.style.opacity="1",i.style.overflow="visible"),l&&(l.style.transform="rotate(0deg)")}if(o){let m=0,u=!1;const n=d=>{m=d.touches?d.touches[0].clientY:d.clientY,u=!0},f=d=>{if(!u)return;u=!1;const S=(d.changedTouches?d.changedTouches[0].clientY:d.clientY)-m;S>30?s():S<-30&&p()};o.addEventListener("touchstart",n,{passive:!0}),o.addEventListener("touchend",f,{passive:!0}),o.addEventListener("mousedown",n),o.addEventListener("mouseup",f),o.onclick=()=>{r?p():s()}}i&&(i.style.maxHeight="600px",i.style.transition="max-height 0.3s ease, opacity 0.3s ease"),e.querySelector("#toggle").onclick=async()=>{ie();try{const m=await se.get(a.id);a.blocked=m.blocked,a.walletBalance=m.walletBalance,R(a)}catch{}if(!a.online&&(a.blocked||(a.walletBalance||0)<=-50)){Ae();return}a.online=!a.online,R(a),c.emit(a.online?"driver:online":"driver:offline",a.id),window.location.reload()},setTimeout(()=>{const m=e.querySelector("#dmap");if(!m)return;const u=(a==null?void 0:a.latitude)||-23.55,n=(a==null?void 0:a.longitude)||-46.63,f=t?15:13;F=L.map(m,{zoomControl:!1,attributionControl:!1}).setView([u,n],f),L.tileLayer(fe(),{subdomains:["0","1","2","3"]}).addTo(F),_e().then(d=>{F.flyTo([d.latitude,d.longitude],f,{duration:1.5}),fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${d.latitude}&lon=${d.longitude}&addressdetails=1`).then(k=>k.json()).then(k=>{const y=k.address||{},_=y.city||y.town||y.village||y.municipality||"Rio de Janeiro",q=y.neighbourhood||y.suburb||"";b=_;const E=e.querySelector("#city-name");E&&(E.textContent=[q,_].filter(Boolean).join(" • ")||"Região desconhecida"),K&&h()}).catch(()=>{const k=e.querySelector("#city-name");k&&(k.textContent="GPS ativo"),b="Rio de Janeiro",K&&h()});const v=t?`<div style="position:absolute;left:0;top:0;pointer-events:none">
           ...
           </div>`;me=L.marker([d.latitude,d.longitude],{icon:L.divIcon({className:"",html:v,iconSize:[0,0],iconAnchor:[0,0]})}).addTo(F),e.querySelector("#btn-recenter").onclick=()=>{const k=me.getLatLng();F.flyTo(k,f,{duration:.8})};const S=e.querySelector("#btn-traffic-dash");if(S&&(S.onclick=()=>{C=!C,localStorage.setItem("movvi_map_traffic",C?"on":"off"),S.innerHTML=`<span class="material-symbols-outlined">${C?"traffic":"layers"}</span>`,S.className=`bg-white/90 dark:bg-background-dark/80 backdrop-blur-md rounded-xl p-3 shadow-lg border border-slate-300 dark:border-primary/20 transition-all ${C?"text-blue-500":"text-slate-400"}`,Me(),P&&V()}),G){const k=Je(),y=e.querySelector("#traffic-card"),_=e.querySelector("#traffic-level"),q=e.querySelector("#traffic-tip"),E=e.querySelector("#traffic-indicator"),j=e.querySelector("#traffic-emoji"),B=e.querySelector("#traffic-fill"),D={livre:{emoji:"🟢",bg:"linear-gradient(135deg,#f0fdf4,#dcfce7)",dotColor:"#22c55e",fillBg:"linear-gradient(90deg,#22c55e,#4ade80)",width:"33%",labelCls:"text-sm font-black text-green-800 leading-none",tipCls:"text-[10px] font-semibold text-green-700/70 mt-1.5 leading-snug line-clamp-2"},moderado:{emoji:"🟡",bg:"linear-gradient(135deg,#fefce8,#fef9c3)",dotColor:"#eab308",fillBg:"linear-gradient(90deg,#eab308,#facc15)",width:"66%",labelCls:"text-sm font-black text-yellow-800 leading-none",tipCls:"text-[10px] font-semibold text-yellow-700/70 mt-1.5 leading-snug line-clamp-2"},intenso:{emoji:"🔴",bg:"linear-gradient(135deg,#fef2f2,#fecaca)",dotColor:"#ef4444",fillBg:"linear-gradient(90deg,#ef4444,#f87171)",width:"100%",labelCls:"text-sm font-black text-red-800 leading-none",tipCls:"text-[10px] font-semibold text-red-700/70 mt-1.5 leading-snug line-clamp-2"}},I=D[k.level]||D.livre;y&&(y.style.background=I.bg),j&&(j.textContent=I.emoji),_&&(_.textContent=k.label,_.className=I.labelCls),q&&(q.textContent=k.tip,q.className=I.tipCls),E&&(E.style.background=I.dotColor),B&&(B.style.background=I.fillBg,B.style.width=I.width),t&&L.circleMarker([d.latitude,d.longitude],{radius:60,color:k.color,fillColor:k.color,fillOpacity:.08,weight:1,opacity:.3}).addTo(F)}Y&&We(d.latitude,d.longitude).then(k=>{if(!k||!k.current_weather)return;const y=k.current_weather,_=Ze[y.weathercode]||"Desconhecido",q=Math.round(y.temperature),E=Math.round(y.windspeed),j=y.weathercode,B=e.querySelector("#weather-temp"),D=e.querySelector("#weather-desc"),I=e.querySelector("#weather-wind"),z=e.querySelector("#weather-icon-emoji"),oe=e.querySelector("#weather-forecast"),O=e.querySelector("#weather-card");if(B&&(B.textContent=q+"°"),D&&(D.textContent=_),I&&(I.textContent=E+" km/h"),j===0||j===1?(z&&(z.textContent="☀️"),O&&(O.style.background="linear-gradient(135deg,#fffbeb,#fef3c7)")):j>=45&&j<=48?z&&(z.textContent="🌫️"):j>=51&&j<=67?(z&&(z.textContent="🌧️"),O&&(O.style.background="linear-gradient(135deg,#f0f9ff,#cfe2ff)")):j>=80&&j<=82?(z&&(z.textContent="⛈️"),O&&(O.style.background="linear-gradient(135deg,#eef2ff,#c7d2fe)")):j>=95&&z&&(z.textContent="⚡"),oe&&k.hourly){const X=new Date().getHours();let pe="";for(let g=X+1;g<Math.min(X+4,(k.hourly.time||[]).length);g++){const w=new Date(k.hourly.time[g]).getHours(),Z=Math.round(k.hourly.temperature_2m[g]);pe+=`<span class="text-[9px] font-bold whitespace-nowrap bg-white/60 px-1.5 py-0.5 rounded-md">${w}h ${Z}°</span>`}oe.innerHTML=pe}}).catch(()=>{})}).catch(()=>{})},100);let b="Rio de Janeiro";function h(){let m=[],u=0;const n=e.querySelector("#news-item"),f=e.querySelector("#news-loading"),d=e.querySelector("#news-counter");function v(y){return`<div class="flex items-start gap-2.5 p-2.5 rounded-xl bg-slate-50 border border-slate-100">
      ...
      </div>`}function S(){!n||m.length===0||(n.style.opacity="0",n.style.transform="translateY(6px)",setTimeout(()=>{n.innerHTML=v(m[u]),n.style.opacity="1",n.style.transform="translateY(0)",d&&m.length>1&&(d.textContent=`${u+1}/${m.length}`,d.classList.remove("hidden"))},250))}function k(){console.log("[News] Fetching for:",b),f&&(f.style.display=""),fetch(`/api/news?city=${encodeURIComponent(b)}`).then(y=>y.json()).then(y=>{if(f&&(f.style.display="none"),!y.items||y.items.length===0){n&&(n.innerHTML='<div class="p-3 rounded-xl bg-slate-50 border border-slate-100"><p class="text-[11px] text-slate-400 font-semibold">Sem notícias de hoje</p></div>');return}m=y.items,u=0,S()}).catch(()=>{f&&(f.style.display="none"),n&&(n.innerHTML='<div class="p-3 rounded-xl bg-slate-50 border border-slate-100"><p class="text-[11px] text-slate-400 font-semibold">Falha ao carregar</p></div>')})}k(),setInterval(()=>{m.length>1&&(u=(u+1)%m.length,S()),u===0&&m.length>0&&k()},4e4)}return e}function Qe(e){const t=document.createElement("div");t.className="view active bg-background-light dark:bg-background-dark";const i=(e.driverPrice||e.price||0).toFixed(2).replace(".",",");let l=Math.floor((e.timeout||15e3)/1e3);t.innerHTML=`
    ...
    <div class="relative mb-10 w-full flex flex-col items-center justify-center">
      ...
      <div class="relative flex items-center justify-center z-10 scale-[1.35] drop-shadow-[0_10px_30px_rgba(0,0,0,0.15)] dark:drop-shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center">
          <span id="tt" class="text-5xl font-black text-slate-900 dark:text-white leading-none tracking-tighter transition-colors duration-500" style="font-family: 'JetBrains Mono', monospace;">${l}</span>
...
</div > `;const o=t.querySelector("#tc"),r=(e.timeout||15e3)/1e3,s=t.querySelector("#tt"),p=t.querySelector("#tt-lbl"),b=t.querySelector("#glow-ring"),h=()=>{o.style.strokeDashoffset=264*(1-l/r);const u=l/r;u<=.3?(o.style.stroke="#ef4444",s.style.color="#ef4444",p.style.color="#ef4444",b.classList.remove("bg-primary/30","bg-[#f97316]/30"),b.classList.add("bg-[#ef4444]/40","animate-[ping_1s_cubic-bezier(0,0,0.2,1)_infinite]")):u<=.6&&(o.style.stroke="#f97316",s.style.color="#f97316",p.style.color="#f97316",b.classList.remove("bg-primary/30"),b.classList.add("bg-[#f97316]/30"))},m=()=>{try{$.pause(),$.currentTime=0}catch{}};try{$.currentTime=0,$.play().catch(u=>console.log("Audio blocked",u))}catch{}return h(),re=setInterval(()=>{l--,l<0&&(l=0),s.textContent=l,h(),l<=0&&(clearInterval(re),m())},1e3),t.querySelector("#ba").onclick=()=>{m(),clearInterval(re),c.emit("order:accept",{driverId:a.id,orderId:e.orderId}),N=e,x(Le,e)},t.querySelector("#bd").onclick=()=>{m(),clearInterval(re),c.emit("order:decline",{driverId:a.id,orderId:e.orderId}),x(T)},t}function Le(e){const t=document.createElement("div");t.className="view active",t.innerHTML=`
<div class="relative flex flex-col" style="height:100dvh">
  ...
  <div class="absolute top-4 right-4 z-[60] flex gap-2">
      <div class="flex flex-col gap-2">
        ...
        <button id="btn-traffic-dash" class="bg-white/90 dark:bg-background-dark/80 backdrop-blur-md rounded-xl p-3 shadow-lg border border-slate-300 dark:border-primary/20 transition-all ${C?"text-blue-500":"text-slate-400"}"><span class="material-symbols-outlined">${C?"traffic":"layers"}</span></button>
      </div>
    </div>
  ...
  <div class="absolute right-4 top-24 z-40 flex flex-col gap-2">
    ...
    <button id="btn-traffic-job" class="size-11 rounded-full bg-white/70 dark:bg-slate-900/60 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.2)] flex items-center justify-center transition-all ${C?"text-blue-500":"text-slate-400"}">
      <span class="material-symbols-outlined text-[24px]">${C?"traffic":"layers"}</span>
    </button>
  </div>
  ...
  <div class="absolute bottom-0 left-0 right-0 p-6 pt-4 border-t border-slate-200 dark:border-white/5 w-full z-40 bg-white dark:bg-background-dark pb-8">
    ...
    <button id="b3" class="w-full bg-slate-900 text-white font-bold py-4 rounded-xl hidden relative z-50 uppercase tracking-widest">Concluir Viagem</button>
...
</div>`,t.querySelector("#btn-menu").onclick=de,t.querySelector("#btn-chat").onclick=()=>{const g=document.getElementById("chat-dot");g&&g.remove(),x(at,e)},t.querySelector("#btn-call").onclick=()=>rt(e.clientId,e.clientName,e.clientPhoto);const i=t.querySelector("#b1"),l=t.querySelector("#b2"),o=t.querySelector("#b3"),r=t.querySelector("#btn-start-nav"),s=t.querySelector("#job-sheet-content"),p=t.querySelector("#job-sheet-handle");let b=!1;function h(){b=!0,s&&(s.style.maxHeight="0px",s.style.opacity="0",s.style.padding="0")}function m(){b=!1,s&&(s.style.maxHeight="600px",s.style.opacity="1",s.style.paddingBottom="0.5rem",s.style.paddingLeft="1.5rem",s.style.paddingRight="1.5rem")}if(p){let g=0,w=!1;const Z=H=>{g=H.touches?H.touches[0].clientY:H.clientY,w=!0},W=H=>{if(!w)return;w=!1;const J=(H.changedTouches?H.changedTouches[0].clientY:H.clientY)-g;J>30?h():J<-30&&m()};p.addEventListener("touchstart",Z,{passive:!0}),p.addEventListener("touchend",W,{passive:!0}),p.addEventListener("mousedown",Z),p.addEventListener("mouseup",W),p.onclick=()=>{b?m():h()}}s&&(s.style.maxHeight="600px"),r.onclick=()=>{r.classList.add("hidden"),i.classList.remove("hidden"),h(),q=!0,v&&v.invalidateSize(),I(),X(y,_)},e.status==="pickup"?(r.classList.add("hidden"),l.classList.remove("hidden")):e.status==="in_progress"&&(r.classList.add("hidden"),o.classList.remove("hidden"),h());let u=null;const n=()=>{u&&navigator.geolocation.clearWatch(u)};i.onclick=()=>{c.emit("order:arrived",{orderId:e.orderId}),i.classList.add("hidden"),l.classList.remove("hidden"),m()},l.onclick=()=>{c.emit("order:start",{orderId:e.orderId}),l.classList.add("hidden"),o.classList.remove("hidden"),h();const g=t.querySelector("#dest-dot");g&&g.classList.replace("border-slate-400","border-primary"),t.querySelector("#route-info").innerHTML='<span class="text-primary font-bold leading-tight drop-shadow-sm">Em viagem para o destino final</span>',X(y,_)},o.onclick=()=>{n(),c.emit("order:complete",{orderId:e.orderId}),N=null,x(Re,e)};const f=e.pickupLat||-23.55,d=e.pickupLon||-46.63;let v=null,S=null,k=null,y=a.latitude||f,_=a.longitude||d,q=!0,E=2,j=null,B=null,D=0;function I(){const g=t.querySelector("#jmap"),w=t.querySelector("#compass-icon");if(g){if(w&&(w.style.transform=`rotate(${-D}deg)`),!q){g.style.transform="rotateX(0deg) rotateZ(0deg) scale(1) translateY(0%)";return}E===2?g.style.transform=`rotateX(62deg) rotateZ(${-D}deg) scale(6) translateY(32%)`:E===1?g.style.transform="rotateX(62deg) rotateZ(0deg) scale(6) translateY(32%)":g.style.transform="rotateX(0deg) rotateZ(0deg) scale(1) translateY(0%)"}}t.querySelector("#btn-perspective").onclick=()=>{q=!0,E=(E+1)%3,I()};const z=t.querySelector("#btn-traffic-job");z&&(z.onclick=()=>{C=!C,localStorage.setItem("movvi_map_traffic",C?"on":"off"),z.innerHTML=`<span class="material-symbols-outlined text-[24px]">${C?"traffic":"layers"}</span>`,z.className=`size-11 rounded-full bg-white/70 dark:bg-slate-900/60 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.2)] flex items-center justify-center transition-all ${C?"text-blue-500":"text-slate-400"}`;const g=fe();v&&v.eachLayer(w=>{w instanceof L.TileLayer&&w.setUrl(g)}),P&&V()});function oe(g){const w=q&&E===2?0:g;return L.divIcon({className:"",html:`
      ...
      `,iconSize:[40,40],iconAnchor:[20,20]})}t.querySelector("#btn-recenter").onclick=()=>{q=!0,E=2,t.querySelector("#btn-recenter").classList.replace("text-slate-600","text-primary"),v.setView([y||a.latitude,_||a.longitude],18),I()};let O=null;async function X(g,w){if(j&&B){const J=g-j,M=(w-B)*Math.cos(g*Math.PI/180);(Math.abs(M)>1e-5||Math.abs(J)>1e-5)&&(D=Math.atan2(M,J)*180/Math.PI)}j=g,B=w,y=g,_=w;const Z=t.querySelector("#route-info");let W=f,H=d;if(N&&N.status==="in_progress"&&(W=e.destinationLat||f,H=e.destinationLon||d),k?(k.setLatLng([g,w]),k.setIcon(oe(q?0:D))):k=L.marker([g,w],{icon:oe(q?0:D)}).addTo(v),q&&(v.setView([g,w],v.getZoom()>16?v.getZoom():18,{animate:!1}),I()),c.emit("driver:location",{driverId:a.id,latitude:g,longitude:w}),O)return;O=setTimeout(()=>{O=null},5e3);try{const M=await(await fetch(`https://router.project-osrm.org/route/v1/driving/${w},${g};${H},${W}?overview=full&geometries=geojson&steps=true`)).json();if(M.routes&&M.routes.length>0){const Be=M.routes[0].geometry.coordinates.map(Q=>[Q[1],Q[0]]);S&&v.removeLayer(S),S=L.polyline(Be,{color:"#3b82f6",weight:6,opacity:.9}).addTo(v);const Ce=Math.round(M.routes[0].duration/60),Ie=(M.routes[0].distance/1e3).toFixed(1);Z.innerHTML=`<span class="material-symbols-outlined text-sm text-blue-500">navigation</span><span class="text-blue-500 font-black uppercase tracking-widest text-[11px]">~${Ce} min • ${Ie}km</span>`;const be=t.querySelector("#nav-instructions");q?(be.classList.remove("opacity-0","-translate-y-40","pointer-events-none"),be.classList.add("opacity-100","translate-y-0")):(be.classList.remove("opacity-100","translate-y-0"),be.classList.add("opacity-0","-translate-y-40","pointer-events-none"));const ve=M.routes[0].legs[0].steps[0],Te=M.routes[0].legs[0].steps[1];if(Te||ve){const Q=Te||ve,he=Q.maneuver.modifier||"straight",$e=Q.maneuver.type||"";let ee="Siga em frente",te="straight";he.includes("left")?(ee="Vire à esquerda",te="turn_left"):he.includes("right")?(ee="Vire à direita",te="turn_right"):he.includes("uturn")&&(ee="Faça o retorno",te="u_turn_left"),$e.includes("arrive")?(ee="Chegando ao destino",te="location_on"):$e.includes("roundabout")&&(ee="Na rotatória",te="roundabout_right");const Oe=Q.name?` na ${Q.name}`:"",ye=ve.distance||0,Fe=ye>1e3?(ye/1e3).toFixed(1)+" km":Math.round(ye)+" m";t.querySelector("#nav-dist").textContent=`EM ${Fe}`,t.querySelector("#nav-desc").textContent=ee+Oe,t.querySelector("#nav-icon").textContent=te}c.emit("driver:eta",{orderId:e.orderId,clientId:e.clientId,etaMinutes:Ce,distanceKm:parseFloat(Ie)});return}}catch(J){console.error("OSRM Nav Error:",J);const M=t.querySelector("#nav-instructions");M&&q&&(M.classList.remove("opacity-0","-translate-y-40","pointer-events-none"),M.classList.add("opacity-100","translate-y-0"),t.querySelector("#nav-dist").textContent="NAVEGANDO",t.querySelector("#nav-desc").textContent="Siga o trajeto no mapa",t.querySelector("#nav-icon").textContent="navigation")}S&&v.removeLayer(S),S=L.polyline([[g,w],[W,H]],{color:"#3b82f6",weight:4,dashArray:"10,8",opacity:.7}).addTo(v);const qe=(Math.sqrt(Math.pow(g-W,2)+Math.pow(w-H,2))*111).toFixed(1);Z.innerHTML=`<span class="material-symbols-outlined text-sm text-blue-500">navigation</span><span class="text-blue-500 font-black uppercase tracking-widest text-[11px]">~${qe}km (linha reta)</span>`}setTimeout(()=>{v=L.map(t.querySelector("#jmap"),{zoomControl:!1,attributionControl:!1}).setView([f,d],15),L.tileLayer(fe(),{subdomains:["0","1","2","3"]}).addTo(v),v.invalidateSize(),L.marker([f,d],{icon:L.divIcon({className:"",html:'<div style="width:20px;height:20px;background:#ffd900;border:3px solid #000;border-radius:50%"></div>',iconSize:[20,20],iconAnchor:[10,10]})}).addTo(v),v.on("dragstart touchstart mousedown",()=>{q&&(q=!1,I())}),v.on("zoomstart",()=>{q&&(q=!1,I())}),X(y,_),navigator.geolocation&&navigator.geolocation.watchPosition&&(u=navigator.geolocation.watchPosition(g=>X(g.coords.latitude,g.coords.longitude),()=>{},{enableHighAccuracy:!0,timeout:5e3,maximumAge:0}))},300);const pe=setInterval(()=>{document.body.contains(t)||(n(),clearInterval(pe))},5e3);return t}function Re(e){const t=document.createElement("div");t.className="view active bg-background-light dark:bg-background-dark";let i=0;const l=(e.driverPrice||e.price||0).toFixed(2).replace(".",",");return t.innerHTML=`
<div class="flex-1 flex flex-col items-center p-6 pt-16">
  ...
  <p class="text-slate-500 text-sm mb-8">Pagamento: R$ ${l}</p>
  <p class="text-slate-500 text-xs font-medium uppercase tracking-wider mb-4">Como foi o cliente?</p>
  <div class="flex gap-2 mb-8" id="stars">${[1,2,3,4,5].map(o=>`<button data-v="${o}" class="size-14 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl flex items-center justify-center hover:border-primary"><span class="material-symbols-outlined text-2xl text-slate-300">star</span></button>`).join("")}</div>
  <button id="done" class="w-full bg-primary text-black font-bold py-4 rounded-xl shadow-lg mt-auto">Concluir</button>
</div>`,t.querySelectorAll("#stars button").forEach(o=>{o.onclick=async()=>{i=parseInt(o.dataset.v),t.querySelectorAll("#stars button span").forEach(r=>{const s=parseInt(r.parentElement.dataset.v);r.className=`material - symbols - outlined text - 2xl ${s<=i?"text-primary":"text-slate-300"} `});try{await we.rate(e.orderId||e.id,i,"driver")}catch{}}}),t.querySelector("#done").onclick=()=>x(T),t}function He(){const e=document.createElement("div");e.className="view active bg-background-light dark:bg-background-dark",e.innerHTML=`
    ...
    <main id="wallet-main" class="flex-1 overflow-y-auto pb-24">
      <div class="flex flex-col items-center justify-center py-20 animate-pulse">
    ...
    </main>`;const t=e.querySelector("#btn-menu-init");t&&(t.onclick=de);const i=async()=>{try{if(!a||!a.id)return;const o=await je("/drivers/"+a.id+"/wallet"),r=o.balance||0,s=o.transactions||[],p=await se.get(a.id);a.walletBalance=r,a.blocked=p.blocked,R(a);const b=(r||0).toFixed(2).replace(".",","),h=(r||0)<0,m=e.querySelector("#wallet-main");m.innerHTML=`
        <div class="p-6 pb-4 bg-white dark:bg-surface-dark/30 border-b border-slate-200 dark:border-white/5 shadow-sm">
          <div class="flex items-start justify-between mb-4">
            <div>
              ...
              <h2 class="text-4xl font-black ${(r||0)<0?"text-red-500 dark:text-red-400":"text-emerald-500"}">R$ ${b}</h2>
            </div>
            <div>
          ...
          <button id="btn-pay-pix" class="w-full ${h?"bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900":"bg-primary text-black"} font-black py-4 rounded-xl shadow-lg active:scale-[0.98] transition-all flex items-center justify-center gap-2 tracking-wide text-sm">
            ${h?"QUITAR DÍVIDA / PAGAR":"ADICIONAR CRÉDITOS"} <span class="material-symbols-outlined text-base">qr_code_2</span>
          </button>
        </div>
            ...
            <div class="p-4">
              ...
              <div id="tx-list" class="space-y-3">
                ${s.length?s.map(d=>{const v=d.type==="fee",S=(d.amount||0).toFixed(2).replace(".",","),k=d.orderValue?`R$ ${d.orderValue.toFixed(2).replace(".",",")}`:"",y=new Date(d.createdAt).toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"});return`
        <div class="p-4 rounded-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 shadow-sm flex flex-col gap-2">
          <div class="flex items-start justify-between">
            ...
            <p class="${v?"text-red-500":"text-green-500"} font-black text-sm">${v?"":"+"}${S}</p>
          </div>
          ${v?`<div class="flex items-center justify-between border-t border-slate-100 dark:border-white/5 pt-2 mt-1">
          ...
          <div class="flex items-center justify-between">
            ...
            </div>`;const u=e.querySelector("#btn-pay-pix");u&&(u.onclick=()=>l(h?Math.abs(r):0));const n=e.querySelector("#btn-save-pix-wallet"),f=e.querySelector("#wallet-pix-input");n&&f&&(n.onclick=async()=>{const d=f.value.trim();n.innerHTML='<div class="size-4 border-2 border-slate-200 border-t-slate-500 rounded-full animate-spin"></div>';try{await je("/drivers/"+a.id,"PUT",{pixKey:d}),a.pixKey=d,R(a),n.innerHTML='<span class="material-symbols-outlined text-sm">check</span>',setTimeout(()=>n.innerHTML="Salvar",2e3)}catch{n.innerHTML="ERRO",setTimeout(()=>n.innerHTML="Salvar",2e3)}})}catch(o){console.error("Wallet Error:",o);const r=o.message||"Erro de conexão";e.querySelector("#wallet-main").innerHTML=`
        <div class="p-20 text-center">
          <p class="text-red-500 text-sm font-black mb-4">${r}</p>
        ...
        </div>`;const s=e.querySelector("#retry-wallet");s&&(s.onclick=i)}},l=o=>{const r=document.createElement("div");r.className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300";const s="00020126800014br.gov.bcb.pix01365b8d0027-c2e5-4110-88a4-86830c98dd760218Pagamento de taxas27600016BR.COM.PAGSEGURO0136AB28099B-3336-4382-8291-2C15A645515A5204899953039865802BR5925LEANDRO PALMEIRA DE SOUZA6014RIO DE JANEIRO62290525PAGS0000000002603092327616304B7EE",b=(o>0?o:20).toFixed(2).replace(".",",");r.innerHTML=`
      <div class="w-full max-w-sm bg-white dark:bg-background-dark rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl slide-in-from-bottom duration-500 animate-in relative overflow-hidden">
        ...
        <div class="flex flex-col items-center gap-4 mb-6">
          <div class="bg-white p-4 rounded-3xl shadow-xl border-4 border-slate-50 relative">
            <img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(s)}" class="w-44 h-44 rounded-xl" />
          ...
          <div class="w-full mt-2">
            ...
            <div class="relative">
              ...
              <input id="pix-amount" type="text" value="${b}" readonly class="w-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-white/10 rounded-2xl pl-12 pr-4 py-4 text-2xl font-black text-slate-900 dark:text-white outline-none" />
            </div>
          </div>
        </div>
      ...
      </div>`,document.body.appendChild(r);const h=()=>{r.classList.remove("animate-in"),r.classList.add("animate-out","fade-out","duration-300"),r.querySelector("div").classList.add("slide-out-to-bottom"),setTimeout(()=>r.remove(),300)};r.querySelector("#close-modal-pix").onclick=h,r.querySelector("#btn-copy-pix").onclick=()=>{navigator.clipboard.writeText(s);const u=r.querySelector("#btn-copy-pix"),n=u.innerHTML;u.innerHTML='Código Copiado! <span class="material-symbols-outlined text-base">check_circle</span>',u.classList.replace("bg-slate-900","bg-emerald-600"),setTimeout(()=>{u.innerHTML=n,u.classList.replace("bg-emerald-600","bg-slate-900")},2e3)};const m=r.querySelector("#btn-confirm-pay");m&&(m.onclick=()=>{h(),x(De),setTimeout(()=>{const u=document.getElementById("chat-input");u&&(u.value=`Olá, acabei de realizar o pagamento de R$ ${b} referente às minhas taxas. Segue o comprovante abaixo:`,u.focus())},500)})};return i(),e}function et(){const e=document.createElement("div");return e.className="view active bg-background-light dark:bg-background-dark",e.innerHTML=`
<header class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-white/10 sticky top-0 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md z-10">
  ...
  <h2 class="font-bold text-black font-bold dark:text-white">Histórico</h2><div class="size-10"></div>
            ...
            <main class="flex-1 overflow-y-auto p-4"><div id="hl"><p class="text-slate-500 text-sm text-center py-8">Carregando...</p></div></main>`,e.querySelector("#btn-menu").onclick=de,(async()=>{try{const t=await we.list({driverId:a.id}),i=e.querySelector("#hl");if(!t.length){i.innerHTML='<p class="text-slate-500 text-sm text-center py-8">Nenhuma corrida</p>';return}i.innerHTML=t.map(l=>`<div class="flex items-center gap-4 p-4 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 mb-3"><div class="size-12 rounded-lg bg-slate-100 dark:bg-white/10 flex items-center justify-center text-slate-600 dark:text-slate-400"><span class="material-symbols-outlined">minor_crash</span></div><div class="flex-1"><h4 class="font-semibold text-sm text-black font-bold dark:text-white">${l.serviceName}</h4><p class="text-xs text-slate-500">${l.pickupAddress||"Local"}</p></div><div class="text-right"><p class="text-xs font-bold ${l.status==="completed"?"text-green-500":l.status==="cancelled"?"text-red-500":"text-primary"}">${l.status}</p></div></div>`).join("")}catch{}})(),e}function tt(){const e=document.createElement("div");e.className="view active bg-background-light dark:bg-background-dark";const i=(a.name||"").replace(/\uFFFD/g,"ã").replace(/Joo/i,"João").replace(/Jo.?o/i,"João"),l=a.photo||"";e.innerHTML=`
<header class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-white/10 sticky top-0 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md z-10 transition-all">
  ...
  <h2 class="font-bold text-black font-bold dark:text-white">Perfil</h2><div class="size-10"></div>
              ...
              <div class="flex flex-col items-center gap-4 mb-8 mt-4 relative">
                <div class="relative group cursor-pointer z-10" id="photo-container">
                  <div class="size-28 rounded-full bg-slate-100 dark:bg-slate-800 border-4 border-white dark:border-background-dark shadow-xl flex items-center justify-center text-slate-400 overflow-hidden relative">
                    ${l?`<img src="${l}" class="w-full h-full object-cover" id="p-img" />`:'<span class="material-symbols-outlined text-[3rem]" id="p-icon">person</span>'}
            ...
            </main>`,e.querySelector("#btn-menu").onclick=de;const o=e.querySelector("#f-input"),r=e.querySelector("#btn-save"),s=e.querySelector("#i-name"),p=e.querySelector("#b-edit-name"),b=e.querySelector("#i-pix"),h=e.querySelector("#b-edit-pix");let m=null;return e.querySelector("#photo-container").onclick=()=>o.click(),o.onchange=u=>{const n=u.target.files[0];if(n){const f=new FileReader;f.onload=d=>{m=d.target.result;const v=e.querySelector(".size-28");v.innerHTML=`<img src="${m}" class="w-full h-full object-cover" id="p-img" />
                       <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10"><span class="material-symbols-outlined text-white">photo_camera</span></div>`,r.classList.remove("hidden")},f.readAsDataURL(n)}},p.onclick=()=>{s.removeAttribute("readonly"),s.focus(),s.classList.add("border-b","border-primary","pb-1"),r.classList.remove("hidden")},h.onclick=()=>{b.removeAttribute("readonly"),b.focus(),b.classList.add("border-b","border-primary","pb-1"),r.classList.remove("hidden")},r.onclick=async()=>{const u=s.value.trim();u&&(a.name=u);const n=b.value.trim();a.pixKey=n,m&&(a.photo=m),R(a);try{await fetch("/api/drivers/"+a.id,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:a.name,photo:a.photo,pixKey:a.pixKey})})}catch{}r.textContent="Salvo!",r.classList.remove("bg-primary"),r.classList.add("bg-green-500","text-white"),setTimeout(()=>{r.classList.add("hidden"),r.textContent="Salvar Alterações",r.classList.add("bg-primary"),r.classList.remove("bg-green-500","text-white"),s.setAttribute("readonly","true"),s.classList.remove("border-b","border-primary","pb-1"),b.setAttribute("readonly","true"),b.classList.remove("border-b","border-primary","pb-1"),V()},1500)},e}function De(){const e=document.createElement("div");e.className="view active bg-background-light dark:bg-background-dark",e.innerHTML=`
    ...
    </form>`,e.querySelector("#bk").onclick=()=>x(T);const t=e.querySelector("#chat-msgs"),i=e.querySelector("#chat-file-input"),l=e.querySelector("#btn-attach");l.onclick=()=>i.click();function o(p){const b=p.from==="driver",h=new Date(p.timestamp).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),m=document.createElement("div");m.className=`flex ${b?"justify-end":"justify-start"} `;let u="";p.file&&(p.file.startsWith("data:image")?u=`<img src="${p.file}" class="max-w-full rounded-lg mb-1 border border-black/10 cursor-pointer" onclick="window.open('${p.file}')" />`:u=`<div class="flex items-center gap-2 p-2 bg-black/5 rounded-lg mb-1"><span class="material-symbols-outlined">description</span><a href="${p.file}" target="_blank" class="text-xs underline">Documento</a></div>`),p.message&&(u+=`<p class="text-sm">${p.message}</p>`),m.innerHTML=`<div class="max-w-[80%] px-4 py-2.5 rounded-2xl ${b?"bg-primary text-black font-bold rounded-br-md shadow-sm":"bg-slate-100 dark:bg-white/10 text-black font-bold dark:text-white rounded-bl-md shadow-sm"}">${u} <p class="text-[9px] mt-1 ${b?"text-black/60":"text-slate-400"} text-right">${h}</p></div>`,t.appendChild(m),t.scrollTop=t.scrollHeight}c.emit("chat:get-history",{driverId:a.id}),c.once("chat:history",({messages:p})=>{t.innerHTML="",p.forEach(o)});const r=p=>{p.driverId===a.id&&p.from==="admin"&&o(p)};c.on("chat:new-message",r);const s=async(p,b=null)=>{c.emit("chat:driver-to-admin",{driverId:a.id,driverName:a.name,message:p,file:b}),o({from:"driver",message:p,file:b,timestamp:new Date().toISOString()})};return e.querySelector("#chat-form").onsubmit=p=>{p.preventDefault();const b=e.querySelector("#chat-input"),h=b.value.trim();h&&(s(h),b.value="")},i.onchange=async p=>{const b=p.target.files[0];if(!b)return;const h=new FileReader;h.onload=()=>{s("",h.result),i.value=""},h.readAsDataURL(b)},e}function at(e){const t=document.createElement("div");t.className="view active bg-background-light dark:bg-background-dark";const i=e.orderId||e.id;t.innerHTML=`
...
</div>`,t.querySelector("#bk").onclick=()=>x(Le,e);const l=t.querySelector("#chat-msgs");function o(n){const f=n.from==="driver",d=new Date(n.timestamp).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),v=document.createElement("div");v.className=`flex ${f?"justify-end":"justify-start"} `;let S;n.type==="audio"&&n.audioData?S=`<audio controls src="${n.audioData}" style="max-width:220px;height:40px"></audio>`:S=`<p class="text-sm">${n.message}</p>`,v.innerHTML=`<div class="max-w-[80%] px-4 py-2.5 rounded-2xl ${f?"bg-primary text-black rounded-br-md":"bg-slate-100 dark:bg-white/10 text-black dark:text-white rounded-bl-md"}">${S} <p class="text-[9px] mt-1 ${f?"text-black/60":"text-slate-400"} text-right">${d}</p></div>`,l.appendChild(v),l.scrollTop=l.scrollHeight}c.emit("order-chat:get-history",{orderId:i}),c.once("order-chat:history",({messages:n})=>{n&&n.forEach(o)});const r=n=>{n.orderId===i&&o(n)};c.on("order-chat:new-message",r),t.querySelector("#chat-form").onsubmit=n=>{n.preventDefault();const f=t.querySelector("#chat-input"),d=f.value.trim();d&&(c.emit("order-chat:driver-to-client",{orderId:i,clientId:e.clientId,driverId:a.id,message:d,type:"text"}),o({from:"driver",message:d,type:"text",timestamp:new Date().toISOString()}),f.value="")};let s=null,p=[];const b=t.querySelector("#btn-audio"),h=t.querySelector("#recording-status"),m=t.querySelector("#btn-stop"),u=t.querySelector("#btn-cancel-audio");return b.onclick=async()=>{try{const n=await navigator.mediaDevices.getUserMedia({audio:!0});s=new MediaRecorder(n),p=[],s.ondataavailable=f=>{f.data.size>0&&p.push(f.data)},s.onstop=()=>{n.getTracks().forEach(f=>f.stop())},s.start(),h.classList.remove("hidden"),b.classList.add("hidden")}catch{alert("Não foi possível acessar o microfone.")}},m.onclick=()=>{!s||s.state!=="recording"||(s.onstop=()=>{s.stream.getTracks().forEach(d=>d.stop());const n=new Blob(p,{type:"audio/webm"}),f=new FileReader;f.onloadend=()=>{const d=f.result;c.emit("order-chat:driver-to-client",{orderId:i,clientId:e.clientId,driverId:a.id,type:"audio",audioData:d}),o({from:"driver",type:"audio",audioData:d,timestamp:new Date().toISOString()})},f.readAsDataURL(n),h.classList.add("hidden"),b.classList.remove("hidden")},s.stop())},u.onclick=()=>{s&&s.state==="recording"&&(s.onstop=()=>{s.stream.getTracks().forEach(n=>n.stop())},s.stop()),p=[],h.classList.add("hidden"),b.classList.remove("hidden")},t}function ze(){if(ue=document.getElementById("app-content"),P=document.getElementById("sidebar"),le=document.getElementById("sidebar-overlay"),!ue||!le){console.error("CRITICAL: DOM elements not found.");return}const t=new URLSearchParams(window.location.search).get("invite")==="pioneer";le.onclick=Se;const i=Ke();i?(a=i,ce(),V(),Pe()):x(t?Ne:ge)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ze):ze();let A=null,ae=null;function rt(e,t,i){ie(),x(Ve,{targetId:e,targetName:t,targetPhoto:i,incoming:!1})}function Ve(e){const t=document.createElement("div");t.className="view active bg-slate-900 text-white",t.innerHTML=`
...
</div>`;const i=t.querySelector("#call-status"),l=t.querySelector("#remote-audio"),o=()=>{U.pause(),U.currentTime=0,ae&&ae.getTracks().forEach(s=>s.stop()),A&&A.close(),A=null,ae=null},r=async()=>{if(A=new RTCPeerConnection({iceServers:[{urls:"stun:stun.l.google.com:19302"}]}),A.onicecandidate=s=>{s.candidate&&c.emit("call:signal",{targetId:e.targetId,signal:{ice:s.candidate}})},A.ontrack=s=>{l.srcObject=s.streams[0],i.textContent="Em Chamada",i.classList.remove("animate-pulse")},ae=await navigator.mediaDevices.getUserMedia({audio:!0}),ae.getTracks().forEach(s=>A.addTrack(s,ae)),!e.incoming){const s=await A.createOffer();await A.setLocalDescription(s),c.emit("call:request",{targetId:e.targetId,userId:a.id,callerName:a.name,callerPhoto:a.photo}),c.emit("call:signal",{targetId:e.targetId,signal:{sdp:s}})}};return e.incoming&&(U.play(),t.querySelector("#btn-accept").onclick=async()=>{U.pause(),U.currentTime=0,await r(),c.emit("call:accept",{targetId:e.targetId})}),t.querySelector("#btn-hangup").onclick=()=>{c.emit("call:end",{targetId:e.targetId}),o(),x(T)},c.off("call:signal").on("call:signal",async({signal:s})=>{if(A||await r(),s.sdp){if(await A.setRemoteDescription(new RTCSessionDescription(s.sdp)),s.sdp.type==="offer"){const p=await A.createAnswer();await A.setLocalDescription(p),c.emit("call:signal",{targetId:e.targetId,signal:{sdp:p}})}}else s.ice&&await A.addIceCandidate(new RTCIceCandidate(s.ice))}),c.off("call:accepted").on("call:accepted",()=>{i.textContent="Em Chamada",i.classList.remove("animate-pulse")}),c.off("call:rejected").on("call:rejected",()=>{o(),x(T)}),c.off("call:ended").on("call:ended",()=>{o(),x(T)}),e.incoming||r(),t}function st(){c.on("call:incoming",e=>{ie(),x(Ve,{targetId:e.fromId,targetName:e.callerName,targetPhoto:e.callerPhoto,incoming:!0})})}const it=ce;ce=()=>{it(),st()};
